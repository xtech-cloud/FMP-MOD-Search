...
namespace XTC.FMP.MOD.Search.LIB.Unity
{
    ...
    public class MyRuntime : MyRuntimeBase
    {
        ...
        private Dictionary<string, string> preloadContentInitialsS_ = new Dictionary<string, string>();
        ...
        public override void Preload(Action<int> _onProgress, Action _onFinish)
        {
            ...
            sequenceBundleMeta.OnFinish = () =>
            {
                ...
                preloads_["ContentInitialsS"] = preloadContentInitialsS_;
        ...
        protected void preloadContentMeta(string _contentUri, Action _onFinish, Action _onError)
        {
            ...
            preloadObjectsPool_.LoadText(filefullpath, null, (_bytes) =>
            {
                try
                {
                    ...
                    var contentMeta = JsonConvert.DeserializeObject<ContentMetaSchema>(json);
                    var initials = NPinyin.Pinyin.GetInitials(contentMeta.alias);
                    preloads_[_contentUri + "/meta.json"] = json;
                    preloadContentInitialsS_[_contentUri] = initials;
                }
                catch (System.Exception ex)
                {
